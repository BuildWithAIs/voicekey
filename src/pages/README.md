...
- `SettingsPage.tsx` - 设置页：语言切换即时生效，其余设置防抖自动保存；在标准与实时识别模型卡片上互斥选择模式并打开各自目录，同时管理模型下载、麦克风、快捷键、开机自启、Linux/Omarchy 集成，以及 DeepSeek 排在首位的共用 LLM 连接和润色、语音输入翻译、选中文本翻译快捷键三个独立开关。启用任一功能时校验连接；目标语言随时可设置。
...
- `settings-config.test.ts` - 设置页原有 ASR 模式选择兼容、润色连接完整性、Provider 切换功能开关 reconcile、连接指纹稳定性/失效与检测缓存 TTL、保存后密钥遮罩与并发编辑保留回归测试。
