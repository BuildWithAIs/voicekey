...
- `SettingsPage.tsx` - 设置页：语言切换即时生效，其余设置防抖自动保存；分别管理经典 SenseVoice 与“Streaming Paraformer + 本地标点”组件，只补下载缺失权重且两套实时权重均 ready 后才能启用互斥模式；同时管理麦克风、低音量模式、版本更新、快捷键、开机自启，以及 OpenAI/DeepSeek/OpenRouter 文本润色与翻译连接、固定模型、密钥按需显示和 Provider 切换校验。
