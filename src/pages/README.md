...
- `SettingsPage.tsx` - 设置页：语言切换即时生效，其余设置防抖自动保存；用产品化文案分别管理经典与实时本地识别模型，支持按需下载、查看/打开统一存储目录、确认后删除，并在删除实时模型时同步关闭实时识别；同时管理麦克风、低音量模式、版本更新、快捷键、开机自启，以及 OpenAI/DeepSeek/OpenRouter 文本润色与翻译连接、固定模型、密钥按需显示和 Provider 切换校验。
