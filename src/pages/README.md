...
- `SettingsPage.tsx` - 设置页：语言切换即时生效，其余设置防抖自动保存；管理标准与实时识别模型、麦克风、快捷键、开机自启、Linux/Omarchy 集成，以及 DeepSeek 排在首位的共用 LLM 连接和润色、语音输入翻译、选中文本翻译快捷键三个独立开关。启用任一功能时校验连接；目标语言随时可设置。
