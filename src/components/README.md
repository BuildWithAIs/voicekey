...
玻璃拟态录音状态浮窗组件：渐变描边 + 随状态变色的环境辉光，麦克风图标随音量呼吸缩放并带涟漪光环。流式模式边说边显示 X-ASR 实时文本（尾部未稳定字符弱化显示 + 流光光标），非流式（SenseVoice）以大波形为主视觉；停止后用 X-ASR final text 替换 partial，并继续显示识别/润色阶段（激活步骤胶囊带流光扫过）。
