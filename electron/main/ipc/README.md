...
- `config-handlers.ts` - 配置读写、受限密钥读取、润色连接校验、TokenDance 浏览器 OAuth 授权建 Key、两种本地识别模式的状态/显式下载/确认后删除、统一模型目录打开、本机逻辑核数与内存读取（供实时识别性能提示，不拦截下载）、X-ASR 预热、空闲状态切换模式时立即释放另一套 ASR worker，以及语言快照查询与广播。目录、删除与 TokenDance 授权操作仅允许设置窗口调用。
