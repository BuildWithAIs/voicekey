...
export function registerLogHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.LOG_GET_TAIL, (_event, options?: LogTailOptions) => {
    const requestedMaxBytes =
      typeof options?.maxBytes === 'number' && Number.isFinite(options.maxBytes)
        ? options.maxBytes
        : LOG_TAIL_MAX_BYTES
    const maxBytes = Math.max(1024, Math.min(requestedMaxBytes, LOG_TAIL_MAX_BYTES * 5))
  ...
  // LOG_WRITE: 写入日志（渲染进程 → 主进程），拒绝形状不合法的载荷
  ipcMain.on(IPC_CHANNELS.LOG_WRITE, (_event, payload: unknown) => {
    if (!payload || typeof payload !== 'object') return
    const { level, message, scope } = payload as Record<string, unknown>
    if (typeof message !== 'string' || message.length === 0) return
    if (typeof level !== 'string' || !LOG_LEVELS.includes(level)) return
    if (scope !== undefined && typeof scope !== 'string') return

    writeLog({
      ...(payload as LogEntryPayload),
