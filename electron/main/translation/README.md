...
- `translator.ts` — Core translation service: captures selected text via Ctrl+C, calls LLM API with a translation system prompt, and replaces the selection via plain-text Ctrl+V (clipboard save/restore). No character-by-character deletion — relies on standard GUI behavior where Ctrl+V replaces selected text.
...
Translator.translate()
  ...
  → clear clipboard and write translation as plain text
