...
- `main.ts` - 应用入口，初始化窗口、托盘、IPC、服务与录音流程；经典本地 ASR Provider 首次使用时再初始化；仅打包后的应用注册系统开机自启，开发环境不会写入裸 Electron 启动项。
...
- `local-asr-manager.ts` - 经典模式的 SenseVoiceSmall int8 ONNX 模型状态检测、原路径按需下载、SHA-256 校验与 worker 线程中文识别；连续 20 分钟无任务时释放模型内存。
- `local-asr-worker.ts` - Worker 线程内创建并缓存 `sherpa-onnx` recognizer，执行模型校验与本地 WAV 转写，避免阻塞主进程。
- `streaming-asr-manager.ts` - 管理 Streaming Paraformer 与 CT-Transformer 标点模型目录，复用已安装权重并只补下载缺少的组件；负责校验、流式会话、PCM 队列、最终标点编排和 Paraformer worker 的 20 分钟空闲卸载。
