...
- `constants.ts` - SenseVoice 与 X-ASR-zh-en 480 ms 的版本、文件大小、SHA-256 和 ModelScope/Hugging Face 双源元数据（运行时由公网国家动态排序），实时识别建议配置（6 逻辑核 / 16GB 档内存），以及录音限制、Provider 固定模型、精简 refine system prompt、术语表与翻译规则。
